...
InitialConditionTitovDemoulin<equationsType, dim>::InitialConditionTitovDemoulin(Parameters<dim>& parameters) :
  ...
  for (unsigned int pp = 0; pp < points.size(); ++pp)
  {
    ...
    */
    ...
    if (r_min>1.0) { // external region
      ...
      value_list[pp][0] = rho_0*exp(-zz*invLg);         // mass density outside
      ...
      value_list[pp][0] = rho_0*exp(-zz*Tc2Tp*invLg);   // mass density in the loop
