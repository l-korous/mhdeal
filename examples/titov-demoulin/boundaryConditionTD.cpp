...
template <int dim>
BoundaryConditionTitovDemoulin<dim>::BoundaryConditionTitovDemoulin(Parameters<dim>& parameters) :
...
***************************************************************************/
template <int dim>
void BoundaryConditionTitovDemoulin<dim>::bc_vector_value(int boundary_no, const Point<dim> &point, InputVector &result, const InputVector &W_plus) const
//void BoundaryConditionTitovDemoulin<dim>::bc_vector_value(const std::vector<Point<dim> > &points, std::vector<std::array<double, Equations<dim>::n_components> >& value_list) const

{
  ...
  */
  ...
  // energy density
  result[4] = pressure / (this->parameters.gas_gamma - 1.0) + B_loc[0] * B_loc[0] + B_loc[1] * B_loc[1] + B_loc[2] * B_loc[2];
