...
template <int dim>
void Equations<EquationsTypeMhd, dim>::JacobiM(double A[3][8][8], Vector<double> lv)
{
  ...
  p = this->parameters.gas_gamma * (v[7] - (v[magX] * v[magX] + v[magY] * v[magY] + v[6] * v[6]) - Uk);
  gmmo = this->parameters.gas_gamma - 1.0;
  Ec1 = (v[3] * v[magY] - v[2] * v[6]) * iRh;
  Ec2 = (v[1] * v[6] - v[3] * v[magX]) * iRh;
  Ec3 = (v[2] * v[magX] - v[1] * v[magY]) * iRh;
  E1 = Ec1; //   + ETA * v[8];
  E2 = Ec2; //   + ETA * v[9];
  E3 = Ec3; //   + ETA * v[10];
  ...
  A[0][1][magX] =  - this->parameters.gas_gamma * v[magX];
  A[0][1][magY] = (1 - gmmo) * v[magY];
  A[0][1][6] = (1 - gmmo) * v[6];
  A[0][1][7] = 0.5 * gmmo;
   // A[0][1][8] = 0;
   // A[0][1][9] = 0;
  ...
  A[0][magY][0] = Ec3 * iRh;
  A[0][magY][1] = v[magY] * iRh;
  A[0][magY][2] =  - v[magX] * iRh;
   // A[0][magY][3] = 0;
  A[0][magY][magX] =  - v[2] * iRh;
  A[0][magY][magY] = v[1] * iRh;
   // A[0][magY][6] = 0;
   // A[0][magY][7] = 0;
   // A[0][magY][8] = 0;
   // A[0][magY][9] = 0;
  ...
  A[0][7][0] = 2 * iRh * (v[magY] * Ec3 - v[6] * Ec2) + v[1] * gmmo * Uk * iRh2 - v[1] * (Uk + p) * iRh2;
  A[0][7][1] = 2 * (v[magY] * v[magY] + v[6] * v[6]) * iRh - 2 * v[1] * gmmo * v[1] * iRh2 + (Uk + p) * iRh;
  A[0][7][2] =  - 2 * v[magX] * v[magY] * iRh - v[1] * 2 * gmmo * v[2] * iRh2;
  A[0][7][3] =  - 2 * v[magX] * v[6] * iRh - v[1] * 2 * gmmo * v[3] * iRh2;
  A[0][7][magX] =  - 2 * this->parameters.gas_gamma * v[magX] * v[1] * iRh + 2 * ( - v[magY] * v[2] - v[6] * v[3]) * iRh;
  A[0][7][magY] =  - 2 * this->parameters.gas_gamma * v[magY] * v[1] * iRh + 2 * (v[magY] * v[1] * iRh - E3);
  A[0][7][6] =  - 2 * this->parameters.gas_gamma * v[6] * v[1] * iRh + 2 * (v[6] * v[1] * iRh + E2);
  A[0][7][7] = this->parameters.gas_gamma * v[1] * iRh;
   // A[0][7][8] = 0;
   // A[0][7][9] = 2 * ETA * v[6];
   // A[0][7][10] =  - 2 * ETA * v[magY];
  ...
  if (dim>1) {
    ...
    A[1][2][magX] = (1 - gmmo) * v[magX];
    A[1][2][magY] =  - this->parameters.gas_gamma * v[magY];
    A[1][2][6] = (1 - gmmo) * v[6];
    A[1][2][7] = 0.5 * gmmo;
     // A[1][2][8] = 0;
     // A[1][2][9] = 0;
    ...
    A[1][magX][0] =  - Ec3 * iRh;
    A[1][magX][1] =  - v[magY] * iRh;
    A[1][magX][2] = v[magX] * iRh;
     // A[1][magX][3] = 0;
    A[1][magX][magX] = v[2] * iRh;
    A[1][magX][magY] =  - v[1] * iRh;
     // A[1][magX][6] = 0;
     // A[1][magX][7] = 0;
     // A[1][magX][8] = 0;
     // A[1][magX][9] = 0;
    ...
    A[1][7][0] = 2 * iRh * ( - v[magX] * Ec3 + v[6] * Ec1) + v[2] * gmmo * Uk * iRh2 - v[2] * (Uk + p) * iRh2;
    A[1][7][1] =  - 2 * v[magX] * v[magY] * iRh - 2 * gmmo * v[1] * v[2] * iRh2;
    A[1][7][2] = 2 * (v[magX] * v[magX] + v[6] * v[6]) * iRh - 2 * v[2] * gmmo * v[2] * iRh2 + (Uk + p) * iRh;
    A[1][7][3] =  - 2 * v[magY] * v[6] * iRh - 2 * gmmo * v[2] * v[3] * iRh2;
    A[1][7][magX] =  - 2 * this->parameters.gas_gamma * v[magX] * v[2] * iRh + 2 * (v[magX] * v[2] * iRh + E3);
    A[1][7][magY] =  - 2 * this->parameters.gas_gamma * v[magY] * v[2] * iRh + 2 * ( - v[magX] * v[1] - v[6] * v[3]) * iRh;
    A[1][7][6] =  - 2 * this->parameters.gas_gamma * v[6] * v[2] * iRh + 2 * (v[6] * v[2] * iRh - E1);
    A[1][7][7] = this->parameters.gas_gamma * v[2] * iRh;
     // A[1][7][8] =  - 2 * ETA * v[6];
     // A[1][7][9] = 0;
     // A[1][7][10] = 2 * ETA * v[magX];
  ...
  if (dim>2) {
    ...
    A[2][3][magX] = (1 - gmmo) * v[magX];
    A[2][3][magY] = (1 - gmmo) * v[magY];
    A[2][3][6] =  - this->parameters.gas_gamma * v[6];
    A[2][3][7] = 0.5 * gmmo;
     // A[2][3][8] = 0;
     // A[2][3][9] = 0;
     // A[2][3][10] = 0;
    ...
    A[2][magX][1] =  - v[6] * iRh;
     // A[2][magX][2] = 0;
    A[2][magX][3] = v[magX] * iRh;
    A[2][magX][magX] = v[3] * iRh;
     // A[2][magX][magY] = 0;
    A[2][magX][6] =  - v[1] * iRh;
     // A[2][magX][7] = 0;
     // A[2][magX][8] = 0;
    ...
    A[2][magY][0] =  - Ec1 * iRh;
     // A[2][magY][1] = 0;
    A[2][magY][2] =  - v[6] * iRh;
    A[2][magY][3] = v[magY] * iRh;
     // A[2][magY][magX] = 0;
    A[2][magY][magY] = v[3] * iRh;
    A[2][magY][6] =  - v[2] * iRh;
     // A[2][magY][7] = 0;
     // A[2][magY][8] = ETA;
     // A[2][magY][9] = 0;
    ...
    A[2][7][0] = 2 * iRh * (v[magX] * Ec2 - v[magY] * Ec1) + v[3] * gmmo * Uk * iRh2 - v[3] * (Uk + p) * iRh2;
    A[2][7][1] =  - 2 * v[magX] * v[6] * iRh - 2 * gmmo * v[1] * v[3] * iRh2;
    A[2][7][2] =  - 2 * v[magY] * v[6] * iRh - 2 * gmmo * v[2] * v[3] * iRh2;
    A[2][7][3] = 2 * (v[magX] * v[magX] + v[magY] * v[magY]) * iRh + 2 * v[3] * gmmo * v[3] * iRh2 + (Uk + p) * iRh;
    A[2][7][magX] =  - 2 * this->parameters.gas_gamma * v[magX] * v[3] * iRh + 2 * (v[magX] * v[3] * iRh - E2);
    A[2][7][magY] =  - 2 * this->parameters.gas_gamma * v[magY] * v[3] * iRh + 2 * (v[magY] * v[3] * iRh + E1);
    A[2][7][6] =  - 2 * this->parameters.gas_gamma * v[6] * v[3] * iRh + 2 * ( - v[magX] * v[1] - v[magY] * v[2]) * iRh;
    A[2][7][7] = this->parameters.gas_gamma * v[3] * iRh;
     // A[2][7][8] = 2 * ETA * v[magY];
     // A[2][7][9] =  - 2 * ETA * v[magX];
     // A[2][7][10] = 0;
