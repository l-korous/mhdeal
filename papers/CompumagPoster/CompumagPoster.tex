...
\headerbox{\textbf{FE Spaces}}{name=2,column=0,row=0,span=2,below=2}{
Since typically, interesting phenomena that we would like to model exhibit rapid changes in density, pressure, and velocity, it is beneficial to allow for discontinuities in these components of the solution - Discontinuous Galerkin method is very promising for this. Magnetic field can potentially change rapidly as well, but as we need to to account for Gauss law $\mathrm{div} B = 0$, we shall be looking for the magnetic field component in a smaller space than that of functions arbitrarily discontinuous on element boundaries - we will use the \textit{Hdiv} space - where continuity of normal component holds also across element boundaries.
}
